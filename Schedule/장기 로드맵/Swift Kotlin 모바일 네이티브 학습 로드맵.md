...
> 운영 변경: 2026-07-22부터 코드 입력 없이 읽기·체크박스 선택 퀴즈로 진행
...
- 병행: Go·React 서비스 확장은 2026-08-09부터 [[Schedule/장기 로드맵/Go React 풀스택 및 서버 환경 학습 로드맵]]의 독립 핵심 축으로 진행
- 현재 개인 학습의 중심: 모바일 네이티브를 유지하면서 업무 스택 학습을 하루 1개 병행
...
- 모든 날에 설명과 코드 조각을 읽고 체크박스 보기에서 답을 고른다.
- 코드 입력·컴파일·실행·메모 없이 재작성은 Daily 완료 기준에서 제외한다.
- 문제는 올바른 설명, 실행 결과, 오류 코드, 올바른 코드, Swift·Kotlin 차이 선택을 섞는다.
- 정답을 먼저 보지 않도록 해설은 `<details>` 안에 둔다.
- Day 번호는 **완료한 새 개념 읽기·선택 워크북 수**를 기준으로 증가한다.
- 워크북이 미완료이면 다음 날 같은 Day를 이월한다.
- 새 개념 Day 5개를 완료할 때마다 [[Templates/Swift Kotlin 주간 복습 퀴즈]]로 누적 복습을 진행한다. 누적 복습은 Day 번호를 증가시키지 않는다.

## 일일 읽기·선택 퀴즈 — 15~25분

1. 새로운 개념 설명과 Swift·Kotlin 비교 포인트 읽기
2. 올바른 개념 설명을 체크박스로 선택하기
3. 제시된 코드의 실행 결과를 선택하기
4. 오류가 있는 코드 또는 올바른 코드를 선택하기
5. Swift와 Kotlin의 차이에 맞는 보기를 선택하기
6. 접힌 정답과 해설을 열어 선택을 확인하기

완료 기준은 다음과 같다.

- 개념 선택 문제 1개
- 코드 읽기 문제 2개 이상
- Swift·Kotlin 차이 문제 1개
- 정답과 해설 확인

코드를 직접 입력하지 않아도 완료할 수 있다. 각 문제에서 단일 선택인지 복수 선택인지 확인하고, 정답을 보기 전에 최소 하나의 체크박스를 선택한다.

## 누적 복습 퀴즈 — 20~30분

1. 최근 완료한 새 개념 Day 5개의 핵심 개념을 섞는다.
2. 개념 설명 선택 2개, 실행 결과 선택 2개, 오류 또는 올바른 코드 선택 2개를 기본으로 한다.
3. Swift·Kotlin 차이 문제를 최소 1개 포함한다.
4. 오답은 해설을 읽은 뒤 같은 문제의 답을 다시 선택한다.
5. 새 코드를 입력하거나 실행하지 않는다.

## 1단계 — 읽기·선택 퀴즈 워크북 84개
...
기존 원본의 권장 학습량은 294일·42주 기준이지만, 현재 운영에서는 1단계 84개를 읽기·선택 퀴즈 단위로 진행하고 Day 5개마다 누적 복습을 배치한다. 날짜보다 새 개념 Day 완료 수와 누적 복습 기록을 기준으로 조정한다.

## 시작점

- 시작일: 2026-07-15
- 읽기·선택 퀴즈 전환일: 2026-07-22
...
- 첫 누적 복습: 새 개념 Day 5개 완료 후 [[Templates/Swift Kotlin 주간 복습 퀴즈]]로 진행한다.

## 완료 기준

- Swift 핵심 언어 개념을 정확히 설명한다.
- Kotlin 코드를 읽고 실행 결과와 올바른 구현을 구분한다.
